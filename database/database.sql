...
-- Tue 27 Sep 2016 04:07:30 PM CEST
...
-- -----------------------------------------------------
-- Table `internal`.`group`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `internal`.`group` ;
...
CREATE TABLE IF NOT EXISTS `internal`.`member_group` (
  ...
  CONSTRAINT `member_groups_ibfk_1`
    ...
    REFERENCES `internal`.`group` (`group_id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT);
...
CREATE TABLE IF NOT EXISTS `internal`.`accounting_account` (
  ...
  `accounting_period` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`entity_id`),
